...
4. After it's been imported into Godot then set the GLSL file for the shader otherwise you'll get an error like Stupid compute shader.gd:135 @ run(): StupidComputeShader: No GLSL/Shader file specified (if this happens just make sure the file is imported then set the Glsl_file var again)
